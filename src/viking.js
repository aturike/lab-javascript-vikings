...
class War {
  constructor() {
    this.vikingArmy = []; //{name, health, str}
    this.saxonArmy = []; //{name, health, str}
  }
  ...
  vikingAttack() {
    for (let i = 0; i < this.saxonArmy.length; i++) {
      if (
        ...
        return this.saxonArmy[i].receiveDamage(this.vikingArmy[i].strength);
      }
    }
  }
  ...
  saxonAttack() {
    for (let i = 0; i < this.vikingArmy.length; i++) {
      if (
        this.vikingArmy[i].receiveDamage(this.saxonArmy[i].strength) ===
        `${this.vikingArmy[i].name} has died in act of combat`
      ) {
